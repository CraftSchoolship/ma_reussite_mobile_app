...
import CalendarLocalConfig from "../../utils/CalendarLocalConfig";
import { formatOdooEvents } from "../../utils/MarkedDatesFormatage";
import { useThemeContext } from "../../hooks/ThemeContext"; // Import du contexte du thème
...
const HomeScreen = () => {
  ...
  const [childrenList, setChildrenList] = useState([]);
  const { selectedChild, setSelectedChild } = useAppContext();
  const { isDarkMode } = useThemeContext(); // Accès au mode sombre
          ...
          <Calendar
            ...
            theme={{
              ...
              backgroundColor: isDarkMode
                ? MA_REUSSITE_CUSTOM_COLORS.Black // Mode sombre
                : MA_REUSSITE_CUSTOM_COLORS.White, // Mode clair
            }}
          />
        </Box>
        <ScrollView flexGrow={1} h={"100%"} w={"90%"} mx={"auto"} mb={"10%"}>
